/*
 * Copyright 2009 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not
 * use this file except in compliance with the License. You may obtain a copy of
 * the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
 * License for the specific language governing permissions and limitations under
 * the License.
 */
package com.google.gwt.user.client.rpc;

import java.util.concurrent.CountDownLatch;

import android.os.AsyncTask;

import com.gdevelop.gwt.syncrpc.SyncProxy;
import com.gdevelop.gwt.syncrpc.spaapptest.MainActivity;
import com.gdevelop.gwt.syncrpc.spaapptest.test.AndroidGWTTestCase;
import com.google.gwt.http.client.Request;
import com.google.gwt.http.client.RequestBuilder;
import com.google.gwt.http.client.RequestException;

/**
 * Tests run-time serialization errors for GWT RPC.
 *
 * Modified by P.Prith in 0.4.4 to utilize Local App Engine server for service
 * through SyncProxy against Test in GWT 2.7.0
 */
public class RunTimeSerializationErrorsTest extends
AndroidGWTTestCase<MixedSerializableEchoServiceAsync> {
	public static MixedSerializableEchoServiceAsync getService() {

		return echoTestService;
	}

	static MixedSerializableEchoServiceAsync echoTestService;

	public RunTimeSerializationErrorsTest() throws InterruptedException {
		super(MainActivity.class);

		setServiceInitTask(new AsyncTask<CountDownLatch, Void, Void>() {
			@Override
			protected Void doInBackground(CountDownLatch... arg0) {
				echoTestService = SyncProxy
						.create(MixedSerializableEchoService.class);
				((ServiceDefTarget) echoTestService)
				.setServiceEntryPoint(getModuleBaseURL() + "echo");
				arg0[0].countDown();
				return null;
			}
		});
	}

	/**
	 * @see com.google.gwt.user.client.rpc.RpcTestBase#setUp()
	 */
	@Override
	protected void setUp() throws Exception {
		SyncProxy.suppressRelativePathWarning(true);
		super.setUp();
	}

	public void testBadSerialization1() {
		delayTestFinishForRpc();
		setTask(new AsyncTask<Void, Void, Void>() {

			@Override
			protected Void doInBackground(Void... arg0) {
				getService().echoVoid(
						new MixedSerializable.NonSerializableSub(),
						new AsyncCallback<MixedSerializable>() {
							@Override
							public void onFailure(Throwable caught) {
								finishTest();
							}

							@Override
							public void onSuccess(MixedSerializable result) {
								fail("RPC request should have failed");
							}
						});
				return null;
			}
		});
	}

	public void testBadSerialization2() {
		final boolean[] callbackFired = new boolean[] { false };

		Request req = getService().echoRequest(
				new MixedSerializable.NonSerializableSub(),
				new AsyncCallback<MixedSerializable>() {
					@Override
					public void onFailure(Throwable caught) {
						callbackFired[0] = true;
					}

					@Override
					public void onSuccess(MixedSerializable result) {
						fail("RPC request should have failed");
					}
				});

		assertTrue(callbackFired[0]); // should have happened synchronously
		assertFalse(req.isPending());
		req.cancel();
	}

	public void testBadSerialization3() throws RequestException {
		final boolean[] callbackFired = new boolean[] { false };

		RequestBuilder rb = getService().echoRequestBuilder(
				new MixedSerializable.NonSerializableSub(),
				new AsyncCallback<MixedSerializable>() {
					@Override
					public void onFailure(Throwable caught) {
						assertFalse("callback fired twice", callbackFired[0]);
						callbackFired[0] = true;
					}

					@Override
					public void onSuccess(MixedSerializable result) {
						fail("RPC request should have failed");
					}
				});

		assertFalse(callbackFired[0]); // should fail when send() is called
		rb.send();
		assertTrue(callbackFired[0]); // should have happened now
	}

	public void testGoodSerialization1() {
		delayTestFinishForRpc();
		setTask(new AsyncTask<Void, Void, Void>() {

			@Override
			protected Void doInBackground(Void... arg0) {
				getService().echoVoid(new MixedSerializable.SerializableSub(),
						new AsyncCallback<MixedSerializable>() {
					@Override
					public void onFailure(Throwable caught) {
						fail(caught.toString());
					}

					@Override
					public void onSuccess(MixedSerializable result) {
						finishTest();
					}
				});
				return null;
			}
		});
	}

	public void testGoodSerialization2() {
		delayTestFinishForRpc();
		setTask(new AsyncTask<Void, Void, Void>() {

			@Override
			protected Void doInBackground(Void... arg0) {
				getService().echoRequest(
						new MixedSerializable.SerializableSub(),
						new AsyncCallback<MixedSerializable>() {
							@Override
							public void onFailure(Throwable caught) {
								fail(caught.toString());
							}

							@Override
							public void onSuccess(MixedSerializable result) {
								finishTest();
							}
						});
				return null;
			}
		});
	}

	public void testGoodSerialization3() {
		delayTestFinishForRpc();
		setTask(new AsyncTask<Void, Void, Void>() {

			@Override
			protected Void doInBackground(Void... arg0) {
				getService().echoVoid(new MixedSerializable.SerializableSub(),
						new AsyncCallback<MixedSerializable>() {
					@Override
					public void onFailure(Throwable caught) {
						fail(caught.toString());
					}

					@Override
					public void onSuccess(MixedSerializable result) {
						finishTest();
					}
				});
				return null;
			}
		});
	}
}