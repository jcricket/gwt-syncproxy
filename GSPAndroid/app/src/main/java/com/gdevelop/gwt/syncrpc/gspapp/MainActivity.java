 ...
 * coding details. This android interface was created from reviewing and
 * integrating ideas found from:
 * http://blog.notdot.net/2010/05/Authenticating-against
 * -App-Engine-from-an-Android-app.
 */
...
import com.gdevelop.gwt.syncrpc.android.AndroidLoginUtils;
...
class ActivateButton extends AsyncTask<MainActivity, Void, MainActivity> {
	...
	protected void onPostExecute(final MainActivity act) {
		...
		this.rpcService.greetServerArr(((EditText) act.findViewById(R.id.input)).getText().toString(),
				new AsyncCallback<ArrayList<String>>() {
					...
					public void onSuccess(final ArrayList<String> result) {
						act.runOnUiThread(new Runnable() {
							@Override
							public void run() {
								final TextView tv = (TextView) act.findViewById(R.id.result);
...
public class MainActivity extends Activity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == MY_REQUEST_ID) {
			if (resultCode == RESULT_OK) {
				Account account = (Account) data.getExtras().get(AndroidLoginUtils.ACCOUNT_KEY);
				try {
					AndroidLoginUtils.loginAppEngine(this, this.listener, account);
					this.waitForCM = true;
				} catch (Exception e) {
					throw new RuntimeException(e);
				}
	...
	protected void onResume() {
		...
		if (this.cm == null && !this.waitForCM) {
			try {
				AndroidLoginUtils.useAccountSelector(true);
				// Test mode
				AndroidLoginUtils.setLoginUrl(BASE_URL, true);
				AndroidLoginUtils.loginAppEngine(this, this.listener, null, MY_REQUEST_ID);
			} catch (Exception e) {
				throw new RuntimeException(e);
			}
		}
