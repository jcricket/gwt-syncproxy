 ...
 * implementation.
 *
 * Overridden in GSP 0.5 to remove dependency of Guava and StreamHtmlParser by
 * Prithviraj
 */
public class SafeHtmlHostedModeUtils {

	/**
	 * Conditionally checks if the provided HTML string is complete (ends in
	 * "inner HTML" context).
	 *
	 * <p>
	 * This check is intended to assert a convention-of-use constraint of
	 * {@link com.google.gwt.safehtml.shared.SafeHtmlBuilder#appendHtmlConstant(String)}
	 * and
	 * {@link com.google.gwt.safehtml.shared.SafeHtmlUtils#fromSafeConstant(String)}
	 * . Since the check is somewhat expensive, it is intended to run only in
	 * the context of unit-tests or test environments, and not in production
	 ...
	 * <li>In server-side code if the property
	 * {@code com.google.gwt.safehtml.ForceCheckCompleteHtml} is set.</li>
	 * <li>In server-side code if {@link #setForceCheckCompleteHtml(boolean)}
	 * has been called with a {@code true} argument.</li>
	 * </ul>
	 *
	 * @param html
	 *            the HTML to check
	...
	public static void maybeCheckCompleteHtml(String html) {
		if (forceCheckCompleteHtml) {
			throw new RuntimeException(
					"This method is short-curcuited on Android and a check will NOT be completed.");
		}
	...
	/**
	 * Returns whether the provided HTML string is complete (ends in
	 * "inner HTML" context).
	 *
	 * <p>
