 ...
 * the License.
 * 
 * This is a modified file to eliminate unneeded dependencies for the project. - Preethum
 */
...
public class UnicodeEscapingTest/* extends GWTTestCase */{
	...
	private static UnicodeEscapingServiceAsync getService() {
		UnicodeEscapingServiceAsync service = GWT
	...
	// // TODO(jat): decide if we really want to specify this behavior since
	// some
					...
					private void nextBatch() {
						...
						if (current < end) {
							...
							getService().getStringContainingCharacterRange(
									current, Math.min(end, current + size),
									this);
						} else if (!fails.isEmpty()) {
							StringBuilder msg = new StringBuilder();
							for (Throwable t : fails) {
